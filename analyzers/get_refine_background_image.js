...
import { getPromptRefineBackgroundImage} from '../modules/prompt_generator.js';
 ...
 */
export async function getRefineBackgroundImage(postData) {

    let backgroundImage = await getAiImage(postData);
...
async function getAiImage(data) {
    ...
    try{
      reference = await formatImage(imageUrl, 'reference');
    ...
    const prompt = getPromptRefineBackgroundImage(); 
